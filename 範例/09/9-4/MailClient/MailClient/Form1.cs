...
namespace MailClient
{
  public partial class Form1 : Form
  {
    ...
    public static void SendCompletedCallback(object sender, AsyncCompletedEventArgs e)
    {
      ...
      // 判斷非同步作業是否已被取消  
      if (e.Cancelled)
        MessageBox.Show("取消非同步傳送郵件.", ".Net Mail Client", MessageBoxButtons.OK, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button1);

      // 判斷非同步作業是否發生錯誤
      if (e.Error != null)
         // 顯示錯誤訊息
        MessageBox.Show("非同步傳送郵件發生錯誤: " + e.Error.ToString(), ".Net Mail Client", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1);
      else
        // 郵件傳送完畢
        MessageBox.Show("非同步傳送郵件成功.", ".Net Mail Client", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
     }
  }
}