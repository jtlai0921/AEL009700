...
namespace SMTPClient
{
  class SMTPClient
  {
    public static TcpClient smtpClient;
    ...
    static void Main(string[] args)
    {
      ...
      try
      {
        ...
        smtpClient = new System.Net.Sockets.TcpClient();

        // 建立用戶端與郵件伺服器連線
        smtpClient.Connect(smtpServerHost);
    ...
    private static bool SMTPResponse(string echo)
    {
      ...
      {
        // 自郵件伺服器接收資料
        int bytesReceived = networkStream.Read(bytes, 0, (int)smtpClient.ReceiveBufferSize);
