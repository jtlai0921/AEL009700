  ...
  public partial class Form1 : Form
  {
    ...
    private void btnHTML_Click(object sender, EventArgs e)
    {
      try
      {
        ...
        clientSocket.Send(Encoding.ASCII.GetBytes(httpRequest));
        ...
        string httpResponse = Encoding.ASCII.GetString(buffer, 0, byteCount);

        while (byteCount > 0)
        {
          byteCount = clientSocket.Receive(buffer, buffer.Length, 0);

          httpResponse = httpResponse + Encoding.ASCII.GetString(buffer, 0, byteCount);
        }

        txtHTML.Text = httpResponse;
      }
      catch (Exception ex)
      {
